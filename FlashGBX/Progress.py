...
class Progress:
    ...
    def SetProgress(self, args):
        ...
        try:
            if not "method" in self.PROGRESS:
                self.PROGRESS = {}
            now = time.time()
            ...
            if args["action"] == "ABORT":
                ...
                if (
                    "method" not in self.PROGRESS
                    or args["action"] == "READ"
                    and self.PROGRESS["method"] in ("SAVE_WRITE", "ROM_WRITE")
                    or args["action"] == "WRITE"
                    and self.PROGRESS["method"]
                    in ("SAVE_READ", "ROM_READ", "ROM_WRITE_VERIFY")
                ):
                    return
                if self.PROGRESS["pos"] > self.PROGRESS["size"]:
                    return
                skip_speed = False
                ...
                if args["action"] in ("READ", "WRITE"):
                    ...
                    if (
                        self.PROGRESS["pos"]
                        == args["pos"] - self.PROGRESS["flash_offset"]
                    ):
                        skip_speed = True
                    ...
                    if "sector_erase_time" in args:
                        if "sector_erase_time" in self.PROGRESS:
                            self.PROGRESS["sector_erase_time"] = (
                                self.PROGRESS["sector_erase_time"]
                                + args["sector_erase_time"]
                            ) / 2
                        else:
                            self.PROGRESS["sector_erase_time"] = args[
                                "sector_erase_time"
                            ]
                    if "sector_pos" in args:
                        if "sector_erase_time" not in args:
                            self.PROGRESS["sector_erase_time"] = 0
                ...
                if (
                    ((now - self.PROGRESS["time_last_emit"]) > 0.06)
                    or "force_update" in args
                    and args["force_update"] is True
                ):
                    self.PROGRESS["time_elapsed"] = now - self.PROGRESS["time_start"]
                    time_delta = now - self.PROGRESS["time_last_update_speed"]
                    pos_delta = (
                        self.PROGRESS["pos"] - self.PROGRESS["bytes_last_update_speed"]
                    )
                    if (
                        time_delta > 0
                        and (time.time() - self.PROGRESS["time_start"] > 2)
                        and "sector_erase_time" not in args
                    ):
                        speed = (pos_delta / time_delta) / 1024
                        if speed > 0 and not skip_speed:
                            if len(
                                self.PROGRESS["speeds"]
                            ) < 40 or not self._is_outlier(
                                speeds=self.PROGRESS["speeds"],
                                new_number=speed,
                                threshold=25,
                            ):
                                self.PROGRESS["speeds"].append(speed)
                            if len(self.PROGRESS["speeds"]) > 50:
                                self.PROGRESS["speeds"].pop(0)
                            if (
                                len(self.PROGRESS["speeds"]) > 1
                                and random.randint(0, 10) == 0
                            ):
                                self.PROGRESS["speeds"].pop(0)
                        if len(self.PROGRESS["speeds"]) > 0:
                            self.PROGRESS["speed"] = statistics.mean(
                                self.PROGRESS["speeds"]
                            )
                        else:
                            self.PROGRESS["speed"] = 0
                    ...
                    if self.PROGRESS["speed"] > 0 and len(self.PROGRESS["speeds"]) > 0:
                        ...
                        self.PROGRESS["time_left"] = (
                            (self.PROGRESS["size"] - self.PROGRESS["pos"])
                            / 1024
                            / total_speed
                        )
                        if self.PROGRESS["sector_erase_time"] > 0:
                            self.PROGRESS["time_left"] += self.PROGRESS[
                                "sector_erase_time"
                            ] * (
                                self.PROGRESS["sector_count"]
                                - self.PROGRESS["sector_pos"]
                            )
                ...
                self.PROGRESS["time_last_update_speed"] = now
                self.PROGRESS["time_left"] = 0
                if self.PROGRESS["time_elapsed"] == 0:
                    self.PROGRESS["time_elapsed"] = 0.001
                self.PROGRESS["speed"] = (
                    self.PROGRESS["size"] / self.PROGRESS["time_elapsed"]
                ) / 1024
                self.PROGRESS["bytes_last_emit"] = self.PROGRESS["size"]
                if "verified" in args:
                    self.PROGRESS["verified"] = args["verified"] == True

                self.PROGRESS["speed"] = min(
