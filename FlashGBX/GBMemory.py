...
class GBMemoryMap:
    ...
    def __init__(self, rom=None, oldmap=None):
        ...
        if rom is not None:
            ...
            if oldmap is not None:
                ...
                self.MAP_DATA[0x6E:0x70] = struct.pack(
                    "=H", write_count
                )  # update write count

    def ParseMapData(self, buffer_map, buffer_rom=None):
        if buffer_map is False or len(buffer_map) < struct.calcsize(
            "=24sHH12s44s18s8sH8s6sH"
        ):
            return False
        data = {}
        num_games = 0
        ...
        if rom_header["game_title"] in (
            ...
            data["title"] = data["title"].decode(
                "SHIFT-JIS"
                if not rom_header["game_title"]
                in ("GBMEM-MENU MMSA", "GBMEM-MENU 256M")
                else "UTF-8",
                "ignore",
            )
            ...
            for i in range(8):
                ...
                if rom_header["game_title"] in ("NP M-MENU MENU", "GBMEM-MENU MMSA"):
                    values = struct.unpack(
                        ...
                        buffer_rom[0x1C000 + (i * 0x200) : 0x1C200 + (i * 0x200)],
                    )
                elif rom_header["game_title"] == "DMG MULTI MENU ":
                    values = struct.unpack(
                        "=BBBHH12s44s384s18s8s1559s16s",
                        buffer_rom[0x1C000 + (i * 0x800) : 0x1C800 + (i * 0x800)],
                    )
                elif rom_header["game_title"] in ("GBMEM-MENU MMSA", "GBMEM-MENU 256M"):
                    values = struct.unpack(
                        "=BBBHH12s44s256s18s8s7s",
                        buffer_rom[0x10010 + (i * 0x160) : 0x10170 + (i * 0x160)],
                    )
                ...
                temp["title"] = data_menu["title"].decode(
                    "SHIFT-JIS"
                    if not rom_header["game_title"]
                    in ("GBMEM-MENU MMSA", "GBMEM-MENU 256M")
                    else "UTF-8",
                ...
                rom_header_game = RomFileDMG(
                    buffer_rom[
                        temp["rom_offset"] : temp["rom_offset"] + temp["rom_size"]
                    ]
                ).GetHeader()
                temp["header"] = rom_header_game
                # if not ("logo_correct" in rom_header_game and rom_header_game["logo_correct"] is True):
                # 	continue
                if rom_header_game != {} and len(buffer_rom) >= (
                    temp["rom_offset"] + temp["rom_size"]
                ):
                    rsval = RomSizes().GetSize(rom_header_game["rom_size_raw"])
                    if rsval is not None:
                        temp["rom_size"] = min(temp["rom_size"], rsval)
                    temp["crc32"] = (
                        zlib.crc32(
                            buffer_rom[
                                temp["rom_offset"] : temp["rom_offset"]
                                + temp["rom_size"]
                            ]
                        )
                        & 0xFFFFFFFF
                    )
                    temp["sha1"] = hashlib.sha1(
                        buffer_rom[
                            temp["rom_offset"] : temp["rom_offset"] + temp["rom_size"]
                        ]
                    ).hexdigest()
                    temp["sha256"] = hashlib.sha256(
                        buffer_rom[
                            temp["rom_offset"] : temp["rom_offset"] + temp["rom_size"]
                        ]
                    ).hexdigest()
                    temp["md5"] = hashlib.md5(
                        buffer_rom[
                            temp["rom_offset"] : temp["rom_offset"] + temp["rom_size"]
                        ]
                    ).hexdigest()
                    if (
                        "db" in rom_header_game
                        and rom_header_game["db"] is not None
                        and rom_header_game["db"]["rc"] == temp["crc32"]
                    ):
            ...
            return data
        # except:
        # 	print(__("Error: Couldn’t parse the “{gb_memory_cartridge}” data.", gb_memory_cartridge="GB Memory Cartridge"))
        # 	return None
    ...
    def ImportROM(self, data):
        ...
        self.IS_MENU = info["rom_header"]["game_title"] in (
            "NP M-MENU MENU",
            "DMG MULTI MENU ",
            "GBMEM-MENU MMSA",
            "GBMEM-MENU 256M",
        ...
        if not self.IS_MENU:
            ...
            if mbc_type == 2:
                ...
                if info["rom_header"]["ram_size_raw"] not in DmgSaveTypes():
                    sram_size = 0
                    sram_type = 0b000
                else:
                    sram_size = DmgSaveTypes(
                        mbc=info["rom_header"]["ram_size_raw"]
                    ).GetSize()
                    if sram_size == 0:
                        sram_type = 0b000
            ...
            info["menu"]["metadata"]["game_code"] = "{:s} -{:4s}-  ".format(
                "CGB" if info["rom_header"]["cgb"] == 0xC0 else "DMG", game_code
            ).encode("ASCII")
            info["menu"]["metadata"]["title"] = (
                info["rom_header"]["game_title"]
                .encode(
                    "SHIFT-JIS"
                    if not info["rom_header"]["game_title"]
                    in ("GBMEM-MENU MMSA", "GBMEM-MENU 256M")
                    else "UTF-8",
                    "ignore",
                )
            ...
            info["menu"]["metadata"]["kiosk_id"] = f"{AppInfo.NAME:s}".encode(
                "ASCII"
            ).ljust(8, b"\xff")
        ...
        elif info["rom_header"]["game_title"] in (
            "NP M-MENU MENU",
            "DMG MULTI MENU ",
            "GBMEM-MENU MMSA",
            "GBMEM-MENU 256M",
        ):
            if info["rom_header"]["game_title"] in (
                "NP M-MENU MENU",
                "GBMEM-MENU MMSA",
            ):
                menu_ver = 0
            ...
            for i in range(8):
                ...
                elif menu_ver == 2:  # custom
                    pos = 0x10010 + (i * 0x160)
                    menu_item = data[pos : pos + 0x160]
                    values = struct.unpack("=BBBHH12s44s256s18s8s7s", menu_item)
                info = dict(zip(keys, values))
                if info["menu_index"] == 0xFF:
                    continue
                ...
                info["ram_data_size"] = self.GetBlockSizeBackup(info["b_size"]) * (
                    8 * 1024
                )
                del info["title_gfx"]
                ...
                info["rom_header"] = RomFileDMG(
                    data[info["rom_data_offset"] : info["rom_data_offset"] + 0x180]
                ).GetHeader()
                if len(info["rom_header"]) == 0:
                    return
                mbc_type = self.MapperToMBCType(info["rom_header"]["mapper_raw"])
                if mbc_type is False:
                    return
                ...
                if mbc_type == 2:
                    ...
                    if (
                        ...
                        or info["rom_header"]["ram_size_raw"] not in DmgSaveTypes()
                    ):
                        sram_size = 0
                        sram_type = 0b000
                    else:
            ...
            for i in range(len(menu_items)):
                pos = i * 3
                self.MAP_DATA[pos : pos + 3] = struct.pack(
                    ">I", menu_items[i]["map"]["raw"]
                )[:3]
            ...
            self.MAP_DATA[0x66:0x6E] = struct.pack(
                "=8s", f"{AppInfo.NAME:s}".encode("ASCII").ljust(8, b"\xff")
            )
