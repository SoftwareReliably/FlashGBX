...
class GBMemoryMap:
	...
	def ParseMapData(self, buffer_map, buffer_rom=None):
		if buffer_map is False or len(buffer_map) < struct.calcsize("=24sHH12s44s18s8sH8s6sH"):
			return False
		data = {}
		num_games = 0
		#try:
		...
		if (rom_header["game_title"] in ("NP M-MENU MENU", "DMG MULTI MENU ", "GBMEM-MENU MMSA", "GBMEM-MENU 256M")):
			data_list = []
			data["game_code"] = data["game_code"].decode("ASCII", "ignore")
			data["title"] = data["title"].decode("SHIFT-JIS" if not rom_header["game_title"] in("GBMEM-MENU MMSA", "GBMEM-MENU 256M") else "UTF-8", "ignore")
			...
			for i in range(8):
				...
				if rom_header["game_title"] in ("NP M-MENU MENU", "GBMEM-MENU MMSA"):
					...
					values = struct.unpack("=BBBHH12s44s256s18s8s7s", buffer_rom[0x10010+(i*0x160):0x10170+(i*0x160)])
				...
				temp["title"] = data_menu["title"].decode("SHIFT-JIS" if not rom_header["game_title"] in("GBMEM-MENU MMSA", "GBMEM-MENU 256M") else "UTF-8", "ignore")
	...
	def ImportROM(self, data):
		...
		self.IS_MENU = (info["rom_header"]["game_title"] in ("NP M-MENU MENU", "DMG MULTI MENU ", "GBMEM-MENU MMSA", "GBMEM-MENU 256M"))
		...
		if not self.IS_MENU:
			...
			if mbc_type == 2:
				...
				if info["rom_header"]["ram_size_raw"] not in DmgSaveTypes():
					...
					sram_size = DmgSaveTypes(mbc=info["rom_header"]["ram_size_raw"]).GetSize()
					if sram_size == 0:
						sram_type = 0b000
			...
			info["menu"]["metadata"]["title"] = info["rom_header"]["game_title"].encode("SHIFT-JIS" if not info["rom_header"]["game_title"] in ("GBMEM-MENU MMSA", "GBMEM-MENU 256M") else "UTF-8", "ignore").ljust(0x2C)
		...
		elif info["rom_header"]["game_title"] in ("NP M-MENU MENU", "DMG MULTI MENU ", "GBMEM-MENU MMSA", "GBMEM-MENU 256M"):
			if info["rom_header"]["game_title"] in ("NP M-MENU MENU", "GBMEM-MENU MMSA"):
				menu_ver = 0
			elif info["rom_header"]["game_title"] == "DMG MULTI MENU ":
				menu_ver = 1
			elif info["rom_header"]["game_title"] in("GBMEM-MENU MMSA", "GBMEM-MENU 256M"):
				menu_ver = 2
			...
			for i in range(8):
				...
				if mbc_type == 2:
					...
					if info["rom_header"]["game_title"] in ("NP M-MENU MENU", "DMG MULTI MENU ", "GBMEM-MENU MMSA", "GBMEM-MENU 256M") or info["rom_header"]["ram_size_raw"] not in DmgSaveTypes():
						sram_size = 0
						sram_type = 0b000
					else:
						sram_size = DmgSaveTypes(mbc=info["rom_header"]["ram_size_raw"]).GetSize()
