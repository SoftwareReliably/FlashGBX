...
class FirmwareUpdater:
    ...
    def WriteFirmware(self, buffer, fncSetStatus):
        if len(buffer) < 0x10000:
            return 3
        if struct.unpack(">I", buffer[0xFFFC:0x10000])[0] != self.CalcChecksum(buffer):
            return 3
        ...
        while True:
            ...
            try:
                ...
                if "Errno 13" in str(e) and platform.system() == "Linux":
                    fncSetStatus(
                        text=__(
                            "No permission to use device! See the {readme} file.",
                            readme="README.md",
                        ),
                        enableUI=True,
                    )
        ...
        def UpdateFirmware(self):
            with zipfile.ZipFile(Path(self.FWUPD.APP_PATH) / "res" / "fw_JoeyJr.zip") as archive:
                ...
                if self.optFW_LK.isChecked():
                    ...
                    if b"Joey Jr" in fw_data and b"FW LK" in fw_data:
                        verified = True
                    ...
                    with archive.open(fn) as f:
                        fw_data = bytearray(f.read())
                    if b"Joey Jr. Firmware" in fw_data:
                        verified = True
                    ...
                    if not firmware_path.name.endswith(".JR"):
                        msgbox = QtWidgets.QMessageBox(
                            ...
                            text=__(
                                ...
                                firmware_jr="FIRMWARE.JR",
                                url='<a href="https://bennvenn.myshopify.com/products/usb-gb-c-cart-dumper-the-joey-jr">https://bennvenn.myshopify.com/products/usb-gb-c-cart-dumper-the-joey-jr</a>',
                            ),
                    ...
                    if (firmware_path.parent / "DEBUG.TXT").exists():
                        msgbox = QtWidgets.QMessageBox(
                            ...
                            windowTitle=AppInfo.NAME,
                            text=__(
                                "The selected file can not be used. Please visit {url} for the latest official firmware updates.",
                                url='<a href="https://bennvenn.myshopify.com/products/usb-gb-c-cart-dumper-the-joey-jr">https://bennvenn.myshopify.com/products/usb-gb-c-cart-dumper-the-joey-jr</a>',
                            ),
                            standardButtons=QtWidgets.QMessageBox.Ok,
                        )
                    ...
                    try:
                        ...
                        if "Firmware" in fw_string:
                            ...
                            if "Jr4Gen3 Firmware" in fw_string:
                                raise ValueError(__("Jr4Gen3 Firmware found"))
                        verified = True
                        ...
                        fw += "<br><br>" + __(
                            "Warning: The selected firmware file couldn’t be confirmed to be valid automatically ({error}). Please double check that this is a valid firmware file for your Joey Jr. If it is invalid or an update for a different device, it may render your device unusable.",
                            error=str(e),
                        )
                    except:
                        verified = False
            ...
            while True:
                ...
                if ret == 1:
                    ...
                    if self.optFW_MSC.isChecked():
                        text += "\n\n" + __(
                            "Please note that you need to manually copy over the {romlist} file. You can find the latest version of it on the Joey Jr website.",
                            romlist="ROMLIST.RAW",
                        )
                ...
                if ret == 4:
                    ...
                    answer = QtWidgets.QMessageBox.information(
                        ...
                        AppInfo.NAME,
                        __(
                            "If your Joey Jr device is currently running the Drag’n’Drop firmware, please select the <b>{mode_txt}</b> (or <b>{mode_txt_2}</b>) file that is stored on the device.",
                            mode_txt="MODE.TXT",
                            mode_txt_2="MODE!.TXT",
                        ),
                    ...
                    path = QtWidgets.QFileDialog.getOpenFileName(
                        self,
                        __(
                            "Choose the {mode_txt} file of your Joey Jr removable drive",
                            mode_txt="MODE.TXT",
                        ),
                        path,
                        "MODE.TXT (MODE*.TXT)",
