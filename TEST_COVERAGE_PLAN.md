...
- Task 1d: added 20 flashcart-configuration tests covering firmware 7/8 pullup commands, firmware 11/12 command acknowledgements, absent/disabled/enabled WR pullups, AGB IRQ feature flags, forced/manual/default-MBC5 selection, unsupported mapper rejection before construction, pulse reset, exact and partial DMG banks, mapper size advisories, default and banked AGB geometry, mapper enable ordering, and consumption of the real helper's configuration by integrated preparation. 1,085 tests pass. Package coverage rose from 69.33% to 69.56% combined (72.72% statements, 60.82% branches), and `LK_Device.py` rose from 53.72% to 54.51%. `_configure_flashcart_for_write` rose from 31.65% to 97.47% combined coverage (100% statements, 92.86% branches); only two branches outside its declared DMG/AGB mode type remain. Ruff passes; no production change was needed. Next: **2a**.
- Task 2a: added 6 modern firmware-updater controller tests covering missing PCB selection, both v1.4 archive routes, canceled preparation, exact writer paths/status callbacks, disabled controls at the write boundary, terminal return codes 1/2/3, restored controls, success-only device clearing/dialog closure, and no retries or false success after failures. 1,091 tests pass. Package coverage rose from 69.56% to 69.84% combined (73.03% statements, 60.99% branches), and `hw_GBxCartRW.py` rose from 60.05% to 64.40%. Modern `FirmwareUpdaterWindow.UpdateFirmware` rose from 0% to 98.51% combined coverage (100% statements, 91.67% branches); only the loop-back for an undocumented writer return code remains. Ruff passes; no production change was needed. Next: **2b**.
- Task 2b: added 12 V13 updater-orchestration tests using generated valid Intel HEX and temporary archives/custom files. They cover bundled custom/official and external selection, exact decoded writer bytes, remembered custom directories, picker cancellation, unexpected filenames, declined confirmation, malformed/oversized/non-ASCII images, missing archive members, missing/unreadable custom inputs, control/progress restoration before disconnect, and finite writer scripts `[1]`, `[2]`, and `[3, 1]` with identical retry payloads. 1,103 tests pass. Package coverage rose from 69.84% to 70.16% combined (73.36% statements, 61.30% branches), and `hw_GBxCartRW.py` rose from 64.40% to 69.55%. `FirmwareUpdaterWindowV13.UpdateFirmware` rose from 0% to 95.12% combined coverage (96.67% statements, 90.91% branches); the remaining bundled-member guard cannot be reached through the method's selections, and coverage reports the executed explicit retry `continue` as a partial loop edge. Ruff passes; no production change was needed. Next: **2c**.
- Task 2c: added 24 firmware-window safeguard and routing tests covering both modern metadata archives, semantic build timestamps, missing PCB selection, modern tenfold and V13 direct progress scaling, text-only status updates, UI re-enabling, enabled/disabled close behavior with a conservative No default, all modern/legacy updater PCB IDs, unsupported IDs, and unavailable optional Qt window classes. 1,127 tests pass. Package coverage rose from 70.16% to 70.44% combined (73.63% statements, 61.61% branches), and `hw_GBxCartRW.py` rose from 69.55% to 73.96%. `SetPCBVersion`, both `SetStatus` methods, both `CloseDialog` methods, and `GetFirmwareUpdaterClass` have 100% statement/branch coverage. Checkpoint file coverage is `LK_Device.py` 54.51%, `hw_GBxCartRW.py` 73.96%, `FlashGBX_GUI.py` 62.86%, and `FlashGBX_CLI.py` 64.18%. Remaining untouched updater functions are constructor/layout and lifecycle helpers (`_CreateCurrentFirmwareGroup`, `_LoadFirmwareInfo`, `_CreateCurrentDeviceGroup`, both constructors, `run`, `hideEvent`, `reject`, V13 selection helpers, `ReadDeviceInfo`, and `ResetAVR`); the two update controllers retain only documented unreachable/coverage-artifact edges. README now records the local 70% milestone and 73% direction. The gate remains 60% pending matching Linux evidence. Ruff passes; no production change was needed. Next: **3a**.
- Task 3a: added 15 batteryless-SRAM parameter cases covering both platform defaults, location boundaries just below/at/above the ROM threshold and beyond all presets, detected and DMG-header preselection, invalid detected-size fallback, valid/malformed saved locations with mixed values and duplicates, remembered-location fallback, preset/custom hexadecimal acceptance, exact persisted settings, cancellation, invalid custom text, and unset mode. Invalid custom text now returns `False` without persisting offset zero, and inconsistent detected parameters reset both location and size before normal fallback selection. 1,142 tests pass. Package coverage rose from 70.44% to 70.96% combined (74.13% statements, 62.18% branches), and `FlashGBX_GUI.py` rose from 62.86% to 65.20%. `_get_default_bl_location_index` reached 100% statement/branch coverage; `GetBLArgs` reached 98.26% combined coverage with 100% statements and 93.75% branches. Ruff, ty, Pyrefly, and Pyright pass. Next: **3b**.
- Task 3b: added 31 save-profile detection and resumption cases covering ordinary bypasses, DMG/AGB batteryless and Photo! detection, write-test bypass, legacy refusal, selected profiles with batteryless metadata, exact read/write resume state including erase, canceled/missing/zero/wrong-type detection results, valid profiles for both platforms, and integrated backup plus erase resumptions with exactly one transfer. Explicit backup destinations now survive profile detection instead of being replaced by an empty path. 1,173 tests pass. Package coverage rose from 70.96% to 71.37% combined (74.48% statements, 62.79% branches), and `FlashGBX_GUI.py` rose from 65.20% to 67.06%. Both `_prepare_save_backup_cartridge` and `_prepare_save_write_cartridge` reached 100% statement/branch coverage; `_ResumeDetectedCartridgeAction` rose from 29.41% to 67.65% combined coverage, leaving flash resumption and no-saved-argument fallback branches outside this task's normal save workflow. Ruff, ty, Pyrefly, and Pyright pass. Next: **3c**.
- Task 3c: added 25 mapper, profile, and boot-logo cases using generated headers and the real DMG/AGB parsers. They cover sentinel and active-detection profile changes, both platforms, recognized/missing ROM sizes, the DMG memory-cart exception, mapper matches and the MBC1/G-MMC1 compatible pair, every manual and forced incompatible choice, valid/exempt logos, exact `0x30`/`0x9C` replacement reads, fix/continue/cancel/missing-file outcomes, and real `FlashROM` refusal paths with no transfer. 1,198 tests pass. Package coverage rose from 71.37% to 71.88% combined (74.94% statements, 63.41% branches), and `FlashGBX_GUI.py` rose from 67.06% to 69.31%. `CartridgeTypeChanged` reached 89.13% combined coverage with 100% statements, `_ConfirmFlashMapper` reached 91.67%, and `_ConfirmFlashBootLogo` reached 95.56% with 100% statements. Remaining edges are non-dict/unknown-mode profile inputs, two mapper display-label branches, and alternate logo-file condition edges. Ruff passes; no production change was needed. Next: **4a**.
- Task 4a: added 19 CLI interaction and routing cases covering empty/first/last/invalid menu answers, malformed/plain/warning/error configuration messages, canceled interactive selection, zero/one/multiple supported platforms, switch and current-mode autodetection, DMG/AGB/default/invalid platform answers, interactive-console interruption, and matching/unmatched fake firmware actions with the requested port and no cartridge read. 1,217 tests pass. Package coverage rose from 71.88% to 72.36% combined (75.37% statements, 64.02% branches), and `FlashGBX_CLI.py` rose from 64.18% to 68.76%. `_SelectMenuAction`, `_PrintConfigMessages`, and `_RunStandaloneAction` reached 100% statement/branch coverage; `run` rose from 38.79% to 86.21% combined coverage. Remaining `run` lines are firmware-menu construction/error handling and the empty-header failure already adjacent to existing header tests. Ruff passes; no production change was needed. Next: **4b**.
- Task 4b: added 16 batteryless restore and refusal cases covering real DMG/AGB region and profile resolution, overwrite acceptance/refusal, missing and permission-denied inputs, missing regions/profiles, unsupported stress tests, erase refusal/acceptance with an exact erased buffer, fixed-5V warnings for both 3.3V profile forms, and warning bypass on voltage-capable devices. Successful writes assert the complete transfer dictionary, and every rejection preserves its input. 1,233 tests pass. Package coverage rose from 72.36% to 72.60% combined (75.56% statements, 64.38% branches), and `FlashGBX_CLI.py` rose from 68.76% to 71.00%. `_BatterylessSRAM` rose from 46.74% to 97.83% combined coverage (100% statements, 93.33% branches); the remaining edges are an action outside restore/erase and a fixed-5V profile that does not require the warning. Ruff passes; no production change was needed. Next: **4c**.
- Task 4c: added 29 boot-logo and save-configuration cases covering valid/exempt logos, missing files, exact DMG/AGB reads, default/yes/no answers, zero/unusable/explicit mappers, MBC2, both MBC7 save variants, TAMA5, MBC6, DMG/AGB batteryless saves, Photo! profile detection, AGB auto/explicit/invalid types, malformed save metadata, and unknown/unset modes. Accepted `BackupRestoreRAM` forwarding asserts the exact mapper/save transfer, while real unknown-mode refusals preserve the input and issue no transfer. 1,262 tests pass. Package coverage rose from 72.60% to 72.84% combined (75.77% statements, 64.74% branches), and `FlashGBX_CLI.py` rose from 71.00% to 73.37%. `_PromptBootLogoFix` rose from 12.12% to 96.97% combined coverage (100% statements, 91.67% branches), leaving only an unknown-platform branch; `_ResolveSaveConfiguration` rose from 62.20% to 100% statement/branch coverage. Checkpoint file coverage is `LK_Device.py` 54.51%, `hw_GBxCartRW.py` 73.96%, `FlashGBX_GUI.py` 69.31%, and `FlashGBX_CLI.py` 73.37%. Adjacent uncovered CLI work remains in `_PromptHeaderChecksumFix` (50%), `_ConfirmSaveAction` (82.22%), and `BackupRestoreRAM` (81.19%), including timestamp/empty-path, unknown-mapper display, RTC, file-error, and debug branches. README now records the 72.84% local measurement and unchanged omissions; the gate stays at 60% pending matching Linux evidence for a ratchet. Ruff passes; no production change was needed. Next: **5a**.
- Task 5a: added 20 generated-data classifier cases for zero/small/unrecognized sizes, real `DmgSaveTypes` mappings, MBC7 256/512-byte overrides, and 128 KiB candidates with mirrored/distinct halves plus early, strided, and late mismatches in both scanned regions. Each case asserts `(save_size, save_type)` and that input data remains unchanged. 1,282 tests pass. Package coverage rose from 72.84% to 73.06% combined (75.93% statements, 65.11% branches), and `LK_Device.py` rose from 54.51% to 55.29%. `_DetectDmgSaveType` reached 94.64% combined coverage (90.62% statements, 100% branches); only its defensive exception handler for invalid non-integer inputs remains uncovered. Ruff and formatting pass; no production change was needed. Next: **5b**.
- Task 5b: added 22 AGB classification cases for FLASH read failure/zero/unknown IDs, known 64/128 KiB chips, bootleg blank/mirrored/distinct banks and malformed probe data, resolved-FLASH bypass, DACS, supported/unknown SRAM sizes, batteryless override metadata, and EEPROM blank/prefix behavior. EEPROM tests assert both full backup requests and no batteryless probe. 1,304 tests pass. Package coverage rose from 73.06% to 73.53% combined (76.34% statements, 65.75% branches), and `LK_Device.py` rose from 55.29% to 56.96%. `_DetectAgbFlashSaveType` reached 97.67% combined coverage (100% statements, 93.75% branches); its only missing edge is the non-bootleg flash size fallback, which no supported chip table entry can take. `_DetectAgbNonFlashSaveType` reached 100% statement/branch coverage. Ruff and formatting pass; no production change was needed. Next: **5c**.
- Task 5c: added 8 flash-profile matching cases covering malformed/empty profiles and commands, absent probe methods, identifier-command and DMG write-pin mismatches, AGB matching regardless of DMG pin, ID prefix match/mismatch, duplicate IDs/probes, catalog ordering, and reset calls only for profiles that define reset commands. 1,312 tests pass. Package coverage rose from 73.53% to 73.77% combined (76.51% statements, 66.18% branches), and `LK_Device.py` rose from 56.96% to 57.78%. `_MatchDetectedFlashTypes` rose from 0% to 98.25% combined coverage (100% statements, 96.43% branches); the sole remaining edge is its match-display branch for modes outside the supported DMG/AGB modes. Ruff and formatting pass; no production change was needed. Next: **5d**.
- Task 5d: added 18 DMG/AGB worker cases covering all 11 successful return fields, progress order, selected profiles and exact save-read arguments, mapper reset and action cleanup, disabled/retail/high-mapper/3D-memory paths, MBC6/MBC7/TAMA5/G-MMC1 behavior, header/flash/save-read failures, raised preparation errors, and auto-power-off enable/support gates. 1,330 tests pass. Package coverage rose from 73.77% to 74.27% combined (76.96% statements, 66.81% branches), and `LK_Device.py` rose from 57.78% to 59.57%. `_DetectCartridge_Worker` rose from 0% to 95.97% combined coverage (97.62% statements, 92.50% branches); remaining edges are defensive `MODE is None` and unsupported-mode paths. The worker now restores the original auto-power-off timeout in `finally` on success, failures, exceptions, and special-mapper returns. It also skips the discarded save read and classifiers for 3D-memory cartridges while returning size 0/type `None`. Checkpoint module coverage: `LK_Device.py` 59.57%, `hw_GBxCartRW.py` 73.96%, `FlashGBX_GUI.py` 69.31%, `FlashGBX_CLI.py` 73.37%. Nearby classifier/matcher coverage is `_DetectDmgSaveType` 94.64% combined (remaining invalid-input exception handling), `_DetectAgbFlashSaveType` 97.67% (remaining chip-table-unreachable fallback), `_DetectAgbNonFlashSaveType` 100% statement/branch, and `_MatchDetectedFlashTypes` 98.25% (remaining unsupported-mode display branch). README records the 74.27% local result and the 73% directional goal surpassed; the 60% gate remains unchanged pending matching Linux evidence for a ratchet, and omitted backends are unchanged. Ruff, formatting, ty, Pyrefly, and Pyright pass. Next: **6a**.
- Task 6a: added RTC dialog extraction and DMG guard/edit cases for device/header failures, absent or empty RTC data, unsupported mappers, cancellation, MBC3/MBC30/MBCX/HuC-3 manual and system time, plus TAMA5 manual year conversion and buffer preservation. Frozen timezone-aware time verifies exact fields, mapper and preserved day values; write success/failure verifies refresh arguments, message category and return value. The GUI module passes 218 tests; the full suite passes 1,353. Package coverage rose from 74.27% to 74.66% combined (77.31% statements, 67.31% branches); `FlashGBX_GUI.py` rose from 69.31% to 71.05%. `_DmgRtcDialogValues` rose from 0% to 100% statement/branch coverage; `EditRTC` rose from 0% to 65.85% combined (69.62% statements, 59.09% branches). Remaining `EditRTC` work is TAMA5 system/leap-year behavior and AGB editing, scheduled for 6b. Ruff, formatting, and `git diff --check` pass; no production change was needed. Next: **6b**.
- Task 6b: added AGB manual year/weekday and system-time cases, including the one-second 2023-to-2024 rollover, plus cancel/no-write behavior. TAMA5 system-time cases assert the two-second adjustment and exact RTC arguments across 2024 leap day, year 2000 (divisible by 400), and non-leap century 2100. The 2100 case exposed that the manual leap-state control stopped at 3 even though system time writes state 4; its range now allows 4. The GUI module passes 224 tests; the full suite passes 1,359. Package coverage rose from 74.66% to 74.83% combined (77.45% statements, 67.56% branches); `FlashGBX_GUI.py` rose from 71.05% to 71.80%. `_DmgRtcDialogValues` remains at 100% statement/branch coverage; `EditRTC` rose from 65.85% to 98.37% combined (100% statements, 95.45% branches), with only the defensive leap-search exhaustion and unsupported-device-mode branch edges uncovered. The four focus files are `LK_Device.py` 59.57%, `hw_GBxCartRW.py` 73.96%, `FlashGBX_GUI.py` 71.80%, and `FlashGBX_CLI.py` 73.37%. README records the 74.83% local result and unchanged backend omissions; the 60% gate stays in place pending matching Linux evidence for a ratchet. Ruff, formatting, `git diff --check`, ty, Pyrefly, and Pyright pass; Pyrefly reports 0 errors with 1 suppressed diagnostic. Round 3 plan complete.
